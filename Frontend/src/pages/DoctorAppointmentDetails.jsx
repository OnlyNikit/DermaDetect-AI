...
export default function DoctorAppointmentDetails() {
  ...
  const runAction = async (path, failMessage) => {
    try {
      ...
      await api.patch(`/api/appointments/${appointmentId}/${path}`);
      await fetchAppointment();
    } catch (err) {
      alert(err.response?.data?.message || failMessage);
    } finally {
      setActionLoading(false);
    }
  };

  const acceptAppointment = () => runAction("accept", "Unable to accept appointment");
  const rejectAppointment = () => runAction("reject", "Unable to reject appointment");
  const completeAppointment = () =>
    runAction("complete", "Unable to complete appointment");

  const saveNotes = async () => {
    try {
      setSavingNotes(true);

      await api.patch(`/api/appointments/${appointmentId}/notes`, {
        doctorNotes: notes,
      });
  ...
  if (loading && !appointment) {
    return <div className="doctor-detail-state">Loading consultation...</div>;
  }
      ...
      <div className="doctor-detail-container">
        ...
        <section className="detail-card">
          ...
          <div className="patient-detail-grid">
            ...
            <div>
              <span>Email</span>
              <strong>{patient.email || "Not available"}</strong>
            </div>
            ...
            <div>
              <span>Weight</span>
              <strong>{patient.weight ? `${patient.weight} kg` : "Not specified"}</strong>
            </div>

            <div>
              <span>Height</span>
              <strong>{patient.height ? `${patient.height} cm` : "Not specified"}</strong>
            </div>
          </div>
        </section>
        ...
        <section className="detail-card report-card">
          ...
          {!assessment ? (
            <div className="no-report">
              ...
              <p>The patient did not attach an AI skin report to this appointment.</p>
            </div>
          ) : (
            <div className="report-content">
              ...
              <div className="prediction-grid">
                <div className="prediction-box">
                  ...
                  <strong>{prediction.disease || "Not available"}</strong>
                </div>
                ...
                <div className="prediction-box">
                  <span>Severity</span>
                  <strong>{prediction.severity || "Not available"}</strong>
                </div>
              ...
              {(prediction.recommendation || assessment.recommendation) && (
                <div className="report-section">
                  <h3>Recommendation</h3>
                  <p>{prediction.recommendation || assessment.recommendation}</p>
                </div>
              )}
              ...
              {prediction.contagious && (
                <div className="report-section">
                  <h3>Contagious</h3>
                  <p>{prediction.contagious}</p>
                </div>
              )}

              <div className="report-section">
                <h3>Patient-reported details</h3>
                ...
                <div className="assessment-details">
                  ...
                  <div>
                    <span>Itching</span>
                    <strong>{assessment.itching || "—"}</strong>
                  </div>

                  <div>
                    <span>Pain / Burning</span>
                    <strong>{assessment.painBurning || "—"}</strong>
                  </div>

                  <div>
                    <span>Changed / Spread</span>
                    <strong>{assessment.changeSpread || "—"}</strong>
                  </div>
                </div>
              </div>
              ...
              {assessment.changeDetails?.length > 0 && (
                <div className="report-section">
                  ...
                  <p>{assessment.changeDetails.join(", ")}</p>
                </div>
              )}
              ...
              <button
                ...
                onClick={() => navigate(`/doctor/assessment/${assessment._id}`)}
              >
                Open full report →
              </button>
            </div>
        ...
        <section className="detail-card">
          ...
          <button className="save-notes-btn" onClick={saveNotes} disabled={savingNotes}>
